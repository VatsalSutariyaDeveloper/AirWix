...
const { SourceMaster } = require("../../../models/masterModels");
const commonQuery = require("../../../helpers/commonQuery");
const validateRequest = require("../../../helpers/validateRequest");
const MODULE = "Source Master";
...
exports.create = async (req, res) => {
  ...
  try {
    ...
    const result = await commonQuery.createRecord(SourceMaster, data);
...
exports.getAll = async (req, res) => {
  try {
    const result = await commonQuery.findAllRecords(SourceMaster, { status: 0 });
...
exports.getById = async (req, res) => {
  try {
    const record = await commonQuery.findOneById(SourceMaster, req.params.id);
...
exports.update = async (req, res) => {
  try {
    const updated = await commonQuery.updateRecordById(SourceMaster, req.params.id, req.body);
...
exports.delete = async (req, res) => {
  try {
    const deleted = await commonQuery.softDeleteById(SourceMaster, req.params.id);
...
exports.updateStatus = async (req, res) => {
  try {
    ...
    const updated = await commonQuery.updateRecordById(SourceMaster, req.params.id, { status });
