// controllers/CRM/master/generalTaskController.js

const { GeneralTask } = require("../../../models/masterModels");
const commonQuery = require("../../../helpers/commonQuery");
const validateRequest = require("../../../helpers/validateRequest");

const MODULE = "General Task";

// Create
exports.create = async (req, res) => {
  const requiredFields = {
    general_task_name: "General Task Name",
    user_id: "User",
    branch_id: "Branch",
    company_id: "Company"
  };
  ...
  try {
    const result = await commonQuery.createRecord(GeneralTask, {
      general_task_name: req.body.general_task_name,
...
exports.getAll = async (req, res) => {
  try {
    const result = await commonQuery.findAllRecords(GeneralTask, { status: 0 });
...
exports.getById = async (req, res) => {
  try {
    const record = await commonQuery.findOneById(GeneralTask, req.params.id);
...
exports.update = async (req, res) => {
  try {
    const updated = await commonQuery.updateRecordById(GeneralTask, req.params.id, req.body);
...
exports.delete = async (req, res) => {
  try {
    const deleted = await commonQuery.softDeleteById(GeneralTask, req.params.id);
...
exports.updateStatus = async (req, res) => {
  try {
    ...
    const updated = await commonQuery.updateRecordById(GeneralTask, req.params.id, { status });
